...
export class CommentService {
  ...
  getReactionLikeCounts(commentId:string):Observable<any>{
    return this.http.get<ApiResponse<any>>(`${this.commentApiUrl}/${commentId}/reactions/like`).pipe(
      map(res=>res.result),
      catchError(this.handleError)
    )
  }
  getReactionDislikeCounts(commentId:string):Observable<any>{
    return this.http.get<ApiResponse<any>>(`${this.commentApiUrl}/${commentId}/reactions/dislike`).pipe(
      map(res=>res.result),
      catchError(this.handleError)
    )
  }
