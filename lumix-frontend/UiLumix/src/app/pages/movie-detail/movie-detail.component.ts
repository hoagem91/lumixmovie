  ...
  private seekToSavedPosition() {
    ...
    this.movieService.continueWatching().subscribe({
      next: (historyItems: WatchHistory[]) => {
        ...
        if (currentMovieHistory && currentMovieHistory.progressInSeconds > 0) {
          setTimeout(() => {
            if (currentMovieHistory && this.videoPlayer.nativeElement) {
              ...
              this.notification.show(`Tiếp tục xem từ ${this.formatTime(currentMovieHistory.progressInSeconds)}`, 'info');
            }
          }, 200);
        }
      },
